...
def get_parent_oid(node: AnyNode) -> typing.Optional[int]:
    parent = node.parent
    while parent and (type(parent) not in [HTMLElement, TextNode, Context] or type(parent) == Context and not parent.render_base):
...
class HTMLElementSerializer(bsdf.Extension):
    ...
    def encode(self, s, v: HTMLElement):
        return {'n': v.tag_name, 'i': v.oid, 'p': get_parent_oid(v), 'a': v.attributes, 'C': v.classes, 't': v.text, 's': str(v.style), 'f': v._set_focus}
