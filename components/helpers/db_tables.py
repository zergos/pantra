...
def find_template(session: Session, table: type[DBTable], suffix: str) -> str | None:
    template = collect_template(f'{table.__qualname__}{suffix}', session)
    if not template:
        for base in table.__bases__:
            if issubclass(base, DBTable):
                if template_name:=find_template(session, base, suffix):
...
def render_list(session: Session, table_name: str, db_name: str = 'db') -> Context:
    ...
    return parent.render(list_template_name, locals=locals)
...
def render_select(session: Session, table_name: str, callback: Callable[[Any], None], db_name: str = 'db') -> Context:
    ...
    return parent.render(select_template_name, locals=locals)
...
def render_form(session: Session, caller: Context, row: DBTable | type[DBTable], values: dict[str, Any] = None) -> Context:
    ...
    return parent.render(form_template_name, locals=locals)
